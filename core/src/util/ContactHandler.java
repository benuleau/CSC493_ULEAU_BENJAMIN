...
public class ContactHandler implements ContactListener
{
    ...
    {
    	if (objFixture.getBody().getUserData() instanceof GroundTile)
    	{
    		Ax player = (Ax)playerFixture.getBody().getUserData();
    	    player.acceleration.y = 0;
    	    player.velocity.y = 0;
    	    player.jumpState = JUMP_STATE.GROUNDED;
    	    playerFixture.getBody().setLinearVelocity(player.velocity);
    	}
    	else if (objFixture.getBody().getUserData() instanceof Log)
    	{
    		...
    		Log log = (Log)objFixture.getBody().getUserData();
    		//world.flagForRemoval(log);
    	}
    }
